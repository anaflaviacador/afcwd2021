...
echo '<form action="https://mailing.anaflaviacador.com/subscribe" method="POST" target="_blank" accept-charset="utf-8">';
	echo '<label for="name"><input type="text" name="name" placeholder="Nome *" required/><i class="fal fa-lock-alt"></i></label>';
	echo '<label for="email"><input type="email" name="email" placeholder="E-mail *" /><i class="fal fa-envelope"></i></label>';
	echo '<label for="Site"><input type="text" name="Site" placeholder="Seu negócio, site ou blog" /><i class="fal fa-home-heart"></i><span>exemplos: Wedding Business; Blog DecorArt.</span></label>';
	...
	echo '<button type="submit" name="submit" class="button" id="submit"/><i class="fas fa-check-circle"></i> Assinar</button>';
echo '</form>';
