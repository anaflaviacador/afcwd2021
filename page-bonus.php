		...
		while (have_posts()) : the_post();
			echo '<section class="container area-artigo" style="margin-top:0">';
				...
				echo '<aside class="colmenor">';
					...
					echo '<p style="font-size:.8em;margin-bottom:10px"><em style="color:var(--cor-negacao)">Embora sejam livres, não são gratuitos</em>. Por ser uma <strong>cortesia</strong>, os plugins ao lado não atualizam automaticamente em seu painel, '.$cliente.'. Para você garantir essa automação, <em style="color:var(--cor-negacao)">o recomendado é comprar o plugin diretamente no site oficial do mesmo</em>, indicado pelo ícone <i style="color:var(--cor-roxo);" class="fas fa-link"></i>. Com isso, além de updates automáticos, você garante o privilégio do suporte do desenvolvedor original, que é uma maravilha! 🛠</p>';
